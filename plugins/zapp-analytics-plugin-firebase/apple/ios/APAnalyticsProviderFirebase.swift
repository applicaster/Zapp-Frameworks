//
//  APAnalyticsProviderFirebas.swift
//  ZappAnalyticsPluginFirebase
//
//  Created by Alex Zchut on 18/05/2021.
...
open class APAnalyticsProviderFirebase: ZPAnalyticsProvider {
            ...
            if let plistDictionary = NSDictionary(contentsOfFile: path) {
                if plistDictionary.allKeys.count > 0 {
                    if FirebaseApp.app() == nil {
                        ...
                        if let people = providerProperties[JsonKeys.sendUserData] as? String {
                            isUserProfileEnabled = people.boolValue()
                        }
                    }
    ...
    override open func trackEvent(_ eventName: String, parameters: [String: NSObject]) {
        ...
        var combinedParameters = ZPAnalyticsProvider.defaultProperties(defaultEventProperties, combinedWithEventParams: parameters)
